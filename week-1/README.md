...
De pipeline staat in [build.yaml](../.github/workflows/build.yaml) en heeft vier jobs. Validate kijkt of `pyproject.toml` en `poetry.lock` kloppen en of de code compileert. Build bouwt het Docker image en pusht het naar Docker Hub. Test start dat image en kijkt of de app antwoordt. Deploy draait op de runner op CN1. Die maakt het pull-secret aan, verwijdert de oude Deployment en past de nieuwe Deployment en de Service toe met `kubectl`. Op een pull request lopen alleen Validate en Build, zonder push en zonder deploy. De pipeline kan ook handmatig gestart worden via Run workflow, alleen op `main`.
